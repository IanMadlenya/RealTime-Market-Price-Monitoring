...
tickers = pd.Series.from_csv('ticker_list.csv',index_col=False)
...
for i in tickers:
    ...
    # NOTE: the following line needs to be corrected to retrieve the options price data
    raw_calls = option.get_call_data(expiry=option.expiry_dates).reset_index()
    raw_puts = option.get_put_data(expiry=option.expiry_dates).reset_index()    
